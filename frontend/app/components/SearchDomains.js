...
function getSearchHistory() {
  if (typeof window === 'undefined') return [];
  try {
    return JSON.parse(localStorage.getItem('domy_history') || '[]').slice(0, 8);
  } catch { return []; }
}
...
export function SearchDomains() {
  ...
  if (sort === 'tld') filtered.sort((a, b) => a.tld.localeCompare(b.tld));
  else if (sort === 'price-asc') filtered.sort((a, b) => (a.price || 999) - (b.price || 999));
          ...
          {taken.length > 0 && (
            <div style={{ marginTop: '12px', display: 'flex', flexDirection: 'column', gap: '6px' }}>
              <p style={{
                ...
                letterSpacing: '0.05em', marginBottom: '4px', marginTop: '8px',
              }}>
                Taken
              </p>
              {taken.map((r) => (
